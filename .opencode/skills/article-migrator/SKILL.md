...
argument-hint: "[源目录] --date=[默认日期] --tags=[标签]"
...
- tags：用户指定，默认 `["extreme programming", "tdd"]`
- published：始终为 `true`
...
1. **识别图片URL**：从 `![](URL)` 或 `![](/images/...)` 中提取
2. **下载图片**：使用 `curl` 下载到临时目录
3. **转换为WebP**：
   ```bash
   cwebp -q 80 [原文件] -o [输出.webp]
   ```
4. **重命名**：使用描述性英文名称（kebab-case）
5. **移动到目标目录**：`public/images/[category]/`
