---
description: 查询 AI 简报相关动态，默认进入查询模式；仅在用户明确要求发布时才进入自动发布链路
---

请加载并执行 `ai-briefing` skill。

默认进入 **AI 简报查询模式**，只有当用户在 `$ARGUMENTS` 中明确表达“发布 / commit / push / 生成并发布”时，才允许进入发布模式。
...
4. 严格遵循 `ai-briefing` skill 内的意图分流规则：
   - 歧义查询 → 查询模式
   - 明确写/起草/生成简报 → 成稿模式
   - 明确发布/commit/push/生成并发布 → 发布模式
5. 不允许绕过 `ai-briefing` 的意图分流直接进入发布链路
...
/ai-radar 帮我起草今天的 AI 简报     # 进入成稿模式，只返回草稿
/ai-radar 生成并发布今天的 AI 简报   # 进入发布模式，审核通过后发布
```
