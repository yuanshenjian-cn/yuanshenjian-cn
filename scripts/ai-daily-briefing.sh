...
LOG_FILE="$LOG_DIR/claude-ai-daily-briefing-$RUN_ID.log"
LOCK_DIR="/tmp/claude-ai-daily-briefing.lockdir"
...
if [ "$DRY_RUN" = "1" ]; then
  PROMPT="/ai-daily-briefing 起草今天的 AI 每日简报，但不要发布、不要 commit、不要 push"
else
  PROMPT="/ai-daily-briefing 生成并发布今天的 AI 每日简报"
fi
