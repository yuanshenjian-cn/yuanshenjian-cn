...
import { aiBriefingRecommendStream, aiChatStream, aiInvestmentBriefingRecommendStream, aiRecommendStream } from "@/lib/ai-client";
...
describe("aiBriefingRecommendStream", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("should post briefing recommend stream request with range context and parse URL references", async () => {
    ...
    await aiBriefingRecommendStream({
      workerUrl: "/api/ai/",
...
describe("aiInvestmentBriefingRecommendStream", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
