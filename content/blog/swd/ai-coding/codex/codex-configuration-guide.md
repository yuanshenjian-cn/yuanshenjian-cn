...
`gpt-5.5` 是官方 bundled catalog 中推荐的默认模型，你可以在配置中按需求切换。这五行配置覆盖了日常使用中最需要调整的四个维度：

| 配置项 | 可选值 | 建议 |
|--------|--------|------|
| **model** | gpt-5.5, o4-mini 等 | 日常编码用 gpt-5.5，复杂推理任务切 o4 |
...
```bash
# 临时切换模型
codex --model gpt-5.5 "Write unit tests"
