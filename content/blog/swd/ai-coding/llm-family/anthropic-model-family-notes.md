...
published: true
brief: >-
   这是一份按代际持续维护的 Anthropic 模型档案，覆盖从 Claude 3 Opus 到 Claude Opus 5.5、Sonnet 5、Fable 5.1 / Mythos 5.1 的主线代际，重点记录官方发布时间、API 价格、编码与 Agent 侧能力，以及后续只追加不删历史的维护规则。
---
...
|------|-------------|---------|-------------|---------|------------------|
| Claude Opus 5.5 | 2026-09-22 | $4 / 1M | $0.20 / 1M | $20 / 1M | 在 Agentic Coding、电脑操作和知识工作上推进 Opus 5，同时降低 token 与任务成本 |
...
| Claude Sonnet 4.5 | 2025-09-29 | $3 / 1M | $0.30 / 1M | $15 / 1M | Sonnet 4 系列首个重大升级，复杂 Agent 与编码最强 Sonnet |
...
<small>*数据来源：Anthropic 官方新闻页、官方 Pricing 页面、官方模型文档与 Release Notes。既有模型数据查询日期为 2026-09-06，Claude Opus 5.5 新增数据查询日期为 2026-09-23。Claude Opus 5.5 于 2026-09-22 发布，输入/输出价格为 $4/$20 每百万 token，缓存读 $0.20/1M、缓存写 $5/1M；官方称其典型工作负载成本比 Opus 5 低 40%。Claude Fable 5.1 / Mythos 5.1 于 2026-09-01 发布，输入/输出价格为 $10/$50 每百万 token，缓存读 $0.25/1M、缓存写 $12.50/1M；Mythos 5.1 仅面向 Project Glasswing 等受信计划。Claude Opus 5 的输入/输出价格为 $5/$25 每百万 token，缓存读 $0.50/1M、缓存写 $6.25/1M，来自官方 pricing 页。Claude Sonnet 5 自 2026-08-10 起将 $2/$10 的 intro 价格改为标准价格，原定 2026-09-01 上调至 $3/$15 未发生；缓存读 $0.20/1M、缓存写 $2.50/1M，来自官方 pricing 页与 Release Notes。Claude Fable 5 / Mythos 5 的缓存读 $1/1M、缓存写 $12.50/1M 来自官方 pricing 页；该模型自 2026-06-12 起被美国政府指令暂停访问，2026-06-30 指令解除，2026-07-01 恢复全球访问。Sonnet 4.6 的缓存读 $0.30/1M、缓存写 $3.75/1M 来自官方定价页；Sonnet 4.5 的缓存读 $0.30/1M，按官方当前 pricing 页修正。部分旧代缓存读价格官方发布页未单列，按“官方未公布”处理。Claude Opus 4.8 与 Opus 4.7 价格均沿用官方 pricing 当前页口径。*</small>

## Claude Opus 5.5：把长程工作做得更快、单价更低

Claude Opus 5.5 于 2026 年 9 月 22 日发布，是 Claude 5.5 家族的首个型号。Anthropic 将它定位在长时间编码 Agent 和知识工作场景，API 型号为 `claude-opus-5-5`。[官方发布页](https://www.anthropic.com/claude-opus-5-5)给出的单价是输入 $4、缓存读取 $0.20、输出 $20 / 百万 token，缓存写入为 $5。

官方公开的几项结果覆盖了这份档案关注的不同工作：

- 编码：Terminal-Bench 4.0 得分 66.4%，Opus 5 为 52.3%；该项使用 xhigh effort。
- 电脑操作：OSWorld 2.0 的 partial score 为 81.8%，Opus 5 为 74.0%。
- 知识工作：GDPval-AA v2.1 得分 1846 Elo，Opus 5 为 1708。

这些分数来自 Anthropic 发布页所列的评测设置，不宜脱离测试条件横向等同为普遍任务成功率。价格方面，输入和输出单价比 Opus 5 低 20%，缓存读取低 60%；官方估算默认设置下的典型任务总成本低约 40%。
...
Claude Opus 5 在 2026 年 7 月 24 日发布。Anthropic 将它定位为 Opus 4.8 之后的明显代际升级，重点不是单纯增加回答长度，而是让模型在深度推理、长程 Agent、编码和知识工作中更高效地把任务做完。

官方资料里最值得记住的变化有几项：

- **长程 Agent 与编码**：官方称 Opus 5 在 Frontier-Bench、CursorBench 等编码和 Agent 评测上显著超过 Opus 4.8，并更擅长持续验证和迭代自己的工作
- **推理控制**：支持 `low`、`medium`、`high`、`xhigh`、`max` 五档 effort，thinking 默认开启，额外计算量在困难任务上更容易转化为更好的结果
- **长上下文与工具协作**：1M token context window 是默认和最大上下文，支持 128K 最大输出；同时支持在对话中增删工具而不使 prompt cache 失效
- **视觉与知识工作**：官方强调它在图表、文档、界面复现、科学研究、表格和幻灯片等任务上都有提升

Opus 5 的 API 价格与 Opus 4.8 相同：输入 $5/1M、输出 $25/1M；缓存写 $6.25/1M，缓存读 $0.50/1M。官方还提供 Claude API 上的 Fast mode，但其输入/输出价格为 $10/$50 每百万 token，不应与标准价格混写。

我对 Opus 5 的理解是：Anthropic 继续把 Opus 的竞争点放在“复杂任务能不能稳定收尾”上，但这次把重点从单次能力拉到了推理预算、验证习惯和长程执行效率。它不是简单替代 Opus 4.8 的换名版本，而是把 Claude 作为工程协作者时最关键的几个环节同时向前推进。

## Claude Sonnet 5：迄今最 Agentic 的 Sonnet，用 Sonnet 的价格逼近 Opus 的能力

Claude Sonnet 5 在 2026 年 6 月 30 日发布，Anthropic 官方定位是“迄今最 Agentic 的 Sonnet 模型”。

它的核心信号很清晰：Sonnet 线正在快速缩小与 Opus 线的能力差距。Sonnet 5 在推理、工具使用、编码和知识工作等 Agentic 关键维度上大幅超越 Sonnet 4.6，在高 effort 设置下部分任务可匹配 Opus 4.8 的水平。

官方给出的关键数据点：

- **编码**：SWE-bench Verified 和 Coding Agent 评测全面超越 Sonnet 4.6，接近 Opus 4.8
- **Agentic Search**：BrowseComp 上 Sonnet 5（高 effort）可匹配 Opus 4.8
...
我对 Sonnet 5 的理解是：它标志着 Anthropic 正在把此前集中在 Opus 线的 Agentic 能力下放到更广泛可及的价位段。如果你此前因为 Opus 的价格门槛而选择 Sonnet 4.6，Sonnet 5 是一次不需要犹豫的升级——它在几乎相同价位上把能力拉到了接近 Opus 4.8 的水平。
...
所以这代的记录有点特殊：它是 Anthropic 能力上限的一次代际级跃升，但发布仅三天后（2026-06-12）被美国政府出口管制指令冻结。2026-06-30 指令解除，2026-07-01 起 Fable 5 恢复全球访问，Mythos 5 也恢复对已获美国批准的国内组织开放。我把它记进档案，是因为 skill 的原则是只追加不删历史——这次发布和后续的政策变化都真实发生了，它的能力定位和价格结构仍然是后续要参照的基线。
...
价格沿用 $3/$15，和 3.7 Sonnet 持平，但能力明显向前。官方当前 pricing 页列出的缓存读价格为 $0.30/1M，缓存写为 $3.75/1M。
...
Anthropic 从 Claude 3 Opus 到 Claude Opus 5、Fable 5 / Mythos 5 的完整主线，核心不是”功能越来越花”，而是”复杂任务越来越稳”。
