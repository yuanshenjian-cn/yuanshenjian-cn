...
> **注**：GPQA Diamond 三家已接近饱和（~94%），竞争分化正转向多步骤任务和真实工程场景。ARC-AGI 数据（*）来自社区整理（@scaling01），非 Anthropic 官方公告。
...
> "低努力的 Opus 4.7 大致相当于中等努力的 Opus 4.6。"

Claude Code 自发布当天起就将**所有套餐的默认 effort 统一设为 `xhigh`**，成为新的默认深度。
...
| `max` | 极端复杂问题，无 token 限制 | 最高（仅当前会话） |
...
response = client.messages.create(
    ...
    effort="xhigh",
    task_budget=50000,  # 建议总 token 目标
    messages=[{
        "role": "user",
...
response = client.messages.create(
    model="claude-opus-4-7",
    thinking={"type": "adaptive"},
    display="summarized",  # opt-in 显示推理摘要
    ...
)
```
...
Opus 4.7 的发布与 Anthropic 的 **Project Glasswing** 计划密切相关。Mythos Preview 是 Anthropic 目前最强大的模型，但因网络安全能力太强而被限制分发。
