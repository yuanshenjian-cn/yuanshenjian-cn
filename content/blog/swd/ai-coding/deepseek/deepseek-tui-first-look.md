...
把 V4 的低价兑现成可用的工程能力，仅凭 API 调用是不够的。一个长会话需要上下文压缩、需要缓存命中率管理、需要对接 MCP 与本地工具、需要跑得动并发子任务、需要在终端里看到模型的思考过程。DeepSeek 官方只提供零散接口，DeepSeek-TUI 把它们串成完整工作流，打包成一个 Rust 单二进制。
