...
| **write** | 创建新文件 | 跟随 `edit` 权限 | 生成代码、创建配置文件 |
| **edit** | 修改文件内容；`apply_patch` / `multiedit` 也归这一类 | allow/deny/ask | 重构代码、修复 Bug |
| **grep** | 搜索文件内容 | allow | 查找函数定义、定位问题代码 |
| **glob** | 列出匹配文件 | allow | 查找特定类型文件 |
| **lsp** | 语言服务分析（实验性） | allow（需 `OPENCODE_EXPERIMENTAL_LSP_TOOL=true`） | 查类型错误、符号关系、诊断问题 |
| **webfetch** | 抓取网页内容 | allow | 获取 API 文档、技术文章 |
| **websearch** | 网络搜索 | allow（需 OpenCode provider 或 `OPENCODE_ENABLE_EXA=1`） | 查找解决方案、技术资料 |
| **skill** | 加载 SKILL.md 技能文件 | allow | 扩展 AI 能力、自定义工作流 |
| **todowrite** | 创建/更新任务清单 | allow | 项目管理、任务跟踪 |
| **question** | 向用户提问 | allow | 获取用户输入、确认操作 |

`write`、`apply_patch` 和 `multiedit` 都属于写入类能力，统一受 `edit` permission 控制。
...
AI 能理解图片中的布局结构，生成对应的 HTML 和 CSS。当然这个需要模型支持视觉能力，`/models` 里当前支持图像输入的模型就很适合。有了这个功能，根据设计稿还原页面的时候特别省事，尤其是那种布局复杂的页面。
...
编辑 `opencode.json`，按新的 `permission` 结构给不同能力设置权限：
...
这样 AI 日常改代码时不用频繁问我，但执行 shell 命令前还是会确认，平衡了效率和安全。`write`、`apply_patch` 和 `multiedit` 也都跟着 `edit` 一起受控，你可以根据信任程度和工作习惯来调。

**使用 oh-my-opencode 扩展：多智能体协作的黑科技**

oh-my-opencode 是社区开发的多智能体协作框架，仓库现在叫 `oh-my-openagent`，但 npm 包和命令名依然保留 `oh-my-opencode`。里面有 Sisyphus、Hephaestus 等多个专业 Agent，能自动协调完成复杂任务。
...
```
请帮我安装和配置 oh-my-opencode，参考这个仓库里的最新安装文档：
...
```bash
# 在目标项目目录中执行
opencode run --command "生成项目 README"

# 进入项目目录后再跑批处理任务
opencode run --command "修复 lint 错误"

# JSON 输出，方便后续处理
opencode run --command "检查代码质量" --format json
```
