---
title: Oh My OpenCode：Atlas vs Sisyphus 双 orchestrator 架构解析
date: '2026-02-24'
tags:
  - 软件开发
  - AI 编程
...
published: true
brief: >-
  Atlas 和 Sisyphus 是 oh-my-openagent 体系里最有代表性的两条编排思路。Sisyphus 偏向全能主协调器，负责意图解析、动态规划和智能委派；Atlas 则更像纯协调指挥家，不直接写代码，专注于阶段编排和验证。两者在代码权限、适用场景、状态管理、并行策略和验证职责等方面有本质差异，分别代表了高级工程师和技术负责人两种不同的 orchestration 哲学。
---

## Atlas vs Sisyphus：OhMyOpenCode 双 orchestrator 架构解析

仓库现在已经更名为 `oh-my-openagent`，但我下面沿用 OmO 这个简称来讲它的双 orchestrator 结构。Atlas 和 Sisyphus 是这套多代理协调系统里最核心的两条编排链路。
...
| **模型要求** | claude-opus-4-7 / kimi-k2.5 / glm-5 | 以当前工作流配置为准 |

**注**：Sisyphus 这条线的当前核心模型链比较明确：`claude-opus-4-7` → `kimi-k2.5` → `glm-5`。Atlas 是否继续保持独立模型链，则更适合以当前 features reference 和工作流配置为准。
...
export const AGENT_MODEL_REQUIREMENTS: Record<string, ModelRequirement> = {
  ...
  atlas: {
    fallbackChain: [
      // Atlas 的当前模型链更适合直接查看 features reference / 工作流配置
    ],
  },
}
```
...
5. 失败的任务使用任务 ID 或当前工作流保留的上下文继续恢复
