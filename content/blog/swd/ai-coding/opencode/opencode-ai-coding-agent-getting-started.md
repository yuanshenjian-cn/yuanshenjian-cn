...
OpenCode 的出现改变了这一局面。它是一款**完全开源**的 AI 编程代理，支持在终端、桌面应用和主流 IDE 中使用。最吸引人的是：它支持 75+ 家模型提供商，你既可以直接连接自己的 provider，也可以先从 `/models` 里挑当前可用的体验模型开始上手。

这篇文章带你全面了解 OpenCode，从安装配置到核心功能，帮助你快速上手这款效率工具。
...
| **模型丰富** | 支持 75+ 提供商，可直接连接自己的 provider，也可以先从 `/models` 中当前可用的体验模型开始 |
...
桌面版直接以官方下载页当前提供的安装方式为准：

- 下载页：[https://opencode.ai/download](https://opencode.ai/download)
- GitHub Releases：[https://github.com/anomalyco/opencode/releases](https://github.com/anomalyco/opencode/releases)

如果你在 macOS 上，也可以直接用 Homebrew 安装：
...
2. **模型选择**：首次启动会提示选择 AI 模型
   - 可以先从 `/models` 里当前标注 **Free** 或可直接使用的体验模型开始
...
启动后输入 `/models` 查看可用模型，优先选择当前标注 **Free** 或可直接使用的体验模型。体验期间通常不需要额外连接供应商或配置 API Key，即可先把工作流跑起来。
...
| **read** | 读取文件内容 | allow |
| **write** | 创建新文件 | 跟随 `edit` 权限 |
| **edit** | 修改文件内容；`apply_patch` / `multiedit` 也归这一类 | allow/deny/ask |
| **grep** | 搜索文件内容 | allow |
| **glob** | 列出匹配文件 | allow |
| **lsp** | 语言服务分析（实验性） | allow（需 `OPENCODE_EXPERIMENTAL_LSP_TOOL=true`） |
| **webfetch** | 抓取网页内容 | allow |
| **websearch** | 网络搜索 | allow（需 OpenCode provider 或 `OPENCODE_ENABLE_EXA=1`） |
| **skill** | 加载 SKILL.md 技能文件 | allow |
| **todowrite** | 创建/更新任务清单 | allow |
| **question** | 向用户提问 | allow |

其中 `write`、`apply_patch` 和 `multiedit` 虽然是不同的写入能力，但都统一受 `edit` permission 控制。
