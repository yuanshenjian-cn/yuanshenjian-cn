...
早期 agent 配置用 `tools: { write: false }` 二态切换工具开关，官方文档已建议 agent 层优先使用 `permission`——后者支持 glob 模式和 allow/ask/deny 三态，表达力更强。在 `permission` 中，`true` 等价于 `{"*": "allow"}`，`false` 等价于 `{"*": "deny"}`。

不过 `tools` 并未完全消失，**MCP 工具的 per-agent 显隐目前仍只能通过 `tools` 字段实现**（详见第七节）。本文除 MCP 章节外，统一使用 `permission`。
...
```markdown
---
name: my-skill                    # 1-64 字符，小写+连字符
description: "具体描述，1-1024 字符"
license: "MIT"                    # 可选
compatibility: "opencode"         # 可选，字符串
metadata:                         # 可选，仅支持字符串到字符串的键值对
  author: "your-name"
---
...
> `metadata` 字段不接受嵌套对象、数组或数字，复杂类型会被忽略或导致加载失败。
...
2. **name 要匹配目录名** — 正则要求 `^[a-z0-9]+(-[a-z0-9]+)*$`
...
> MCP 工具以服务器名为前缀注册（如 `github_create_issue`、`github_list_prs`），用 glob `"github_*"` 可匹配该服务器的所有工具。注意：`mcp.<name>.enabled: false` 会停掉服务器进程，agent 层无法再启用；要做 per-agent 显隐，需保持服务器 `enabled: true`，再用 `tools` 字段控制可见性。`tools` 虽已 deprecated（见第三节），但 MCP 工具级开关目前仍主要靠它。
...
| Session | `session.created`、`session.updated`、`session.deleted`、`session.idle`、`session.status`、`session.error`、`session.diff`、`session.compacted` |
| Shell | `shell.env` |
| Todo | `todo.updated` |
| Tool | `tool.execute.before`、`tool.execute.after` |
| TUI | `tui.prompt.append`、`tui.command.execute`、`tui.toast.show` |
| 实验性 | `experimental.session.compacting`（替换默认压缩 prompt） |

事件名和签名可能随版本调整，开发 plugin 前建议查看 `@opencode-ai/plugin` 包导出的 TypeScript 类型定义。
