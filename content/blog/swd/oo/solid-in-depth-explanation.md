...
date: '2019-08-15'
brief: >-
  SOLID五大原则的创业故事解读：ISP（接口隔离原则）如袁Sir为不同顾客提供不同工具箱，不要强迫使用者依赖不需要的接口；LSP（里氏替换原则）如徒弟继承师傅手艺但修改风格会导致合作失败，强调子类替换父类行为需一致；DIP（依赖倒置原则）如通过服务清单依赖抽象而非具体执行者，从依赖实现转向依赖抽象；SRP（单一职责原则）如部门拆分各司其职，关注点分离提高效率；OCP（开闭原则）如引入读书大使对扩展开放对修改关闭。SOLID帮助设计高内聚低耦合的软件，降低维护成本。
tags:
  - 软件开发
  - 面向对象
published: true
...
![接口隔离原则的工具箱示例](/images/swd/oo/oo-isp.webp)
...
![里氏替换原则的木匠继承示例](/images/swd/oo/oo-lsp.webp)
...
![依赖倒置原则的部门服务清单示例](/images/swd/oo/oo-dip.webp)
...
![单一职责原则的部门拆分示例](/images/swd/oo/oo-srp.webp)
...
![开闭原则的读书大使示例](/images/swd/oo/oo-ocp.webp)
