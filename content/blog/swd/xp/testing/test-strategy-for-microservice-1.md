...
date: '2017-07-05'
brief: >-
  系统架构从单体应用演变到SOA再到微服务架构，解决了扩展性、维护性、部署发布等瓶颈，但也带来了服务拆分、设计、测试的复杂性。单体应用的测试策略简单直接：足够单元测试、一定Service测试、少量E2E测试。微服务架构下，系统集成测试面临环境维护成本高、问题难定位、运行速度慢等痛点。Pair集成测试提出积小成多理念保证服务俩俩集成可靠。引入Contract概念的集成测试通过前后端共同定义API协议来支持并行开发，但仍缺乏自动化监控机制来同步契约变更，这就是CDCT要解决的问题。
tags:
  - 软件开发
  - 敏捷测试
published: true
...
![单体应用持续集成和测试策略图](/images/swd/xp/testing/single-app-ci.webp)
...
![四个服务组成的微服务架构图](/images/swd/xp/testing/microservice-architecture.webp)
...
![服务两两集成测试策略图](/images/swd/xp/testing/pair-integration-test.webp)
...
![引入 Contract 概念的集成测试架构图](/images/swd/xp/testing/contract-concept-test.webp)
