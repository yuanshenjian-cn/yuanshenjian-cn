...
published: true
brief: >-
  简单设计之识别重复通过清扬与袁帅的代码探索游戏，系统讲解了六种常见重复类型：实现过程重复（IDE可直接提示的代码克隆）、功能语义重复（异曲同工的类或方法）、语法语义重复（重复的条件分支）、执行逻辑重复（重复的条件判断）、配置信息重复、邪恶注释重复。文章强调消除重复的前提是准确识别重复类型，不同类型需采用不同策略，过度消除重复可能违背SRP原则，识别能力需要通过大量Code Review和刻意练习来培养。
---

![画板](/images/xp/simple-design/simple-design-card.webp)
...
![画板](/images/xp/simple-design/simple-design-principles.webp)
...
![画板](/images/xp/simple-design/duplication-types.webp)