...
date: '2020-03-11'
brief: >-
  契约式设计（DbC）探讨模型的有效性问题。本文通过Rectangle/Square继承案例揭示LSP（里氏替换原则）的核心：一个模型的有效性只能通过客户程序来表现。孤立看Square和Rectangle各自有效，但建立继承关系后，Square重写setWidth/setHeight方法破坏了用户对Rectangle的行为假设。强调IS-A关系是基于行为方式的而非静态属性，子类不应改变父类的行为契约，否则会导致模型失效。
tags:
  - extreme programming
  - dbc
published: true
...
![dbc-metaphor.png](/images/xp/dbc-metaphor.webp)
