...
date: '2020-09-23'
brief: >-
  本文翻译自Rouan Wilsenach的文章。生产环境总会出错，但这不一定是坏事——这是了解系统的好机会。"生产环境中的QA"是指团队密切关注生产环境中系统的行为，以便提高系统功能的整体质量。收集生产数据的关键成功指标是那些能支付账单的事情。关键技术包括：Logging（日志记录，提供可搜索的实时数据）、Metrics（指标统计，聚合系统事件和数据）、APIs（从现有工具API获取数据）。通过建立自动化修复系统、设置关键成功指标警报，团队可以更深入地了解系统面临的实际问题，并学习提高质量的新方法。
tags:
  - agile
  - agile-testing
published: false
...
![](https://martinfowler.com/articles/qa-in-production/qa-in-prod.webp)
