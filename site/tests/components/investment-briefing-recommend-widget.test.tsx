...
const aiInvestmentBriefingRecommendationStreamMock = vi.fn();
...
vi.mock("@/lib/ai-client", async (importOriginal) => {
  ...
  return {
    ...actual,
    aiInvestmentBriefingRecommendationStream: (...args: unknown[]) => aiInvestmentBriefingRecommendationStreamMock(...args),
  };
});

describe("InvestmentBriefingRecommendWidget", () => {
  beforeEach(() => {
    aiInvestmentBriefingRecommendationStreamMock.mockReset();
  ...
  it("输入主题后调用 investment-briefing-recommend scene", async () => {
    aiInvestmentBriefingRecommendationStreamMock.mockImplementationOnce(async ({ onEvent }: { onEvent: (event: unknown) => void }) => {
    ...
    await waitFor(() => expect(aiInvestmentBriefingRecommendationStreamMock).toHaveBeenCalledWith(expect.objectContaining({
