...
const aiArticleRecommendationStreamMock = vi.fn();
...
vi.mock("@/lib/ai-client", async (importOriginal) => {
  ...
  return {
    ...actual,
    aiArticleRecommendationStream: (...args: unknown[]) => aiArticleRecommendationStreamMock(...args),
  };
});
...
describe("AiRecommendWidget", () => {
  beforeEach(() => {
    aiArticleRecommendationStreamMock.mockReset();
  ...
  it("提交新搜索时保留旧结果，直到新结果返回前不闪空", async () => {
    ...
    aiArticleRecommendationStreamMock
    ...
    await waitFor(() => expect(aiArticleRecommendationStreamMock).toHaveBeenCalledTimes(2));
  ...
  it("流式开始后失败时保留已流出的 answer，并展示统一错误文案", async () => {
    aiArticleRecommendationStreamMock.mockImplementationOnce(
