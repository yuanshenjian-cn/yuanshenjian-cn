import type {
  ...
  PageResponse,
  PageStreamEvent,
  ArticleRecommendationStreamOptions,
  RecommendReference,
  RecommendResponse,
...
export async function aiBriefingRecommendationStream(options: AiBriefingRecommendationStreamOptions): Promise<void> {
  const workerUrl = options.workerUrl.replace(/\/$/, "");
  const response = await fetch(`${workerUrl}/chat/stream`, {
    method: "POST",
    credentials: "include",
...
export async function aiInvestmentBriefingRecommendationStream(options: InvestmentBriefingRecommendationStreamOptions): Promise<void> {
  const workerUrl = options.workerUrl.replace(/\/$/, "");
  const response = await fetch(`${workerUrl}/chat/stream`, {
    method: "POST",
    credentials: "include",
