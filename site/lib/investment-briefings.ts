...
import { investmentBriefingsDir, repoRoot } from "@/lib/workspace-paths";

const EXCERPT_LENGTH = 180;
const investmentBriefingsDirectory = investmentBriefingsDir;
...
function parseInvestmentBriefingFile(filePath: string): InvestmentBriefing | null {
  try {
    ...
    if (!isValidFrontmatter(data)) {
      console.warn(`[InvestmentBriefings] Skipping invalid briefing: ${path.relative(repoRoot, filePath)}`);
      return null;
    }
    ...
    console.error(`[InvestmentBriefings] Error parsing ${path.relative(repoRoot, filePath)}:`, error);
    return null;
  }
}
