...
export async function generateMetadata({ params }: Props) {
  ...
  const post = slug === "latest" ? getAllPosts()[0] ?? null : getPostBySlug(slug);
  ...
  const shareTitle = isLatestRoute ? "文章 · Latest | YSJ" : post.title;
...
export default async function PostPage({ params }: Props) {
  ...
  const shareTitle = slug === "latest" ? "文章 · Latest | YSJ" : post.title;
