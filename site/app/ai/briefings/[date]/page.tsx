...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `${config.site.url}${isLatestRoute ? "/ai/briefings/latest" : briefing.url}`;
...
export default async function BriefingDetailPage({ params }: Props) {
  ...
  const url = `${config.site.url}${date === "latest" ? "/ai/briefings/latest" : briefing.url}`;
        ...
        <div className="max-w-6xl mx-auto lg:flex lg:justify-center lg:gap-8">
          <div className="w-full max-w-2xl mx-auto lg:mx-0 overflow-hidden">
            <nav className="mb-8 text-sm text-muted-foreground">
              <Link href="/ai/briefings" className="inline-flex items-center gap-1.5 transition-colors hover:text-foreground">
            ...
            <header className="mb-10">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                ...
                <ArticleViewTracker slug={`ai-briefing-${briefing.slug}`} />
              </div>
