...
经过讨论，AI 简报不作为顶部导航独立入口，也不混入普通文章流，而是作为 `/ai` 页面上的“时效内容层”：进入 AI 页面先看到最新一期简报摘要，再继续浏览下方长期沉淀的 AI 专栏。

## 2. 目标

1. 顶部导航保持不变，仍然只有 `AI` 入口。
2. 在 `/ai` 页面顶部新增“最新一期 AI 简报”摘要卡片。
3. 新增 AI 简报往期列表页 `/ai/briefings`，按日期倒序排列。
4. 新增 AI 简报详情页 `/ai/briefings/[date]`，页面简洁、无评论。
5. 在 `/ai/briefings` 顶部提供简报域内的 AI 推荐入口，支持主题输入与快捷日期范围。
6. AI 简报不进入首页最新文章、`/articles`、RSS、全站搜索、首页 AI 推荐。
7. 将旧的 AI 雷达 skill/workflow 改造为发布工作流，并统一为 `ai-briefing`。
...
- 不改变现有顶部导航结构。
- 不把 AI 简报注册为现有 AI 专栏。
- 不让 AI 简报混入普通博客文章列表、RSS 或全站搜索。
- 不让首页 AI 推荐默认推荐 AI 简报。
- 首版不在 AI 简报详情页接入页面级 AI 助手。
- 不做自定义日期范围选择，首版只做快捷范围：今天、近 3 天、近 7 天、近 30 天。
- 不生成 `.mdx` 简报文件；AI 简报统一使用 `.md`。
...
- `/ai`：AI 主题首页，顶部展示最新一期 AI 简报摘要卡，下方展示现有 AI 专栏卡片。
- `/ai/briefings`：AI 简报往期列表页，顶部提供简报推荐入口，下方按日期倒序展示全部已发布简报。
- `/ai/briefings/[date]`：AI 简报详情页，路由参数直接使用 `YYYY-MM-DD`。

### 4.3 静态导出要求

本站使用 `output: 'export'`，因此简报详情动态路由必须显式支持静态生成：

- `app/ai/briefings/[date]/page.tsx` 使用 `generateStaticParams()` 从 `content/ai-briefings` 生成所有日期参数。
- `app/ai/briefings/[date]/page.tsx` 设置 `dynamicParams = false`。
- 没有匹配简报时使用 `notFound()`。
- `/ai/briefings` 和 `/ai/briefings/[date]` 都生成 metadata 与 canonical。
- `app/sitemap.ts` 增加 `/ai/briefings` 与所有简报详情 URL；不加入 RSS。
...
原因：用户已明确要求 AI 简报不进入普通文章流、RSS、全站搜索和首页 AI 推荐。若继续放入 `content/blog`，需要在多个现有入口增加排除逻辑，容易漏出；独立内容目录更符合“AI 频道内独立内容域”的边界。
...
当还没有任何简报时，卡片展示空状态：说明“AI 简报即将上线”，并保留“查看往期”入口但禁用或指向空列表页。

### 6.2 `/ai/briefings` 往期列表页
...
- 推荐结果只来自 AI 简报，不访问普通文章索引。

### 6.3 `/ai/briefings/[date]` 详情页

详情页复用底层 MDX 渲染能力，但不直接复用普通文章容器。当前 `ArticleContent` 会挂评论和普通文章导航，不适合简报详情页。

页面包含：

- 返回 `/ai/briefings` 的面包屑。
- 标题、日期、摘要、标签。
- MDX 正文。
- 分享按钮。
- 上一期 / 下一期导航。
...
该索引只包含 AI 简报元数据，不混入 `public/ai-data/index.json`。
...
返回模式沿用现有 SSE：先输出自然语言推荐理由，再在末尾返回 references。references 指向 `/ai/briefings/[date]`。

前端新增专用组件，例如 `components/briefings/briefing-recommend-widget.tsx`，它只在 `/ai/briefings` 使用，并通过新的 `aiBriefingRecommendStream()` 调用 Worker。现有 `AiRecommendWidget` 和首页 `recommend` scene 不改语义。
...
- 页面、数据索引和内容文件随主站构建发布。
- `/ai/briefings` 顶部 AI 推荐入口依赖新版 Worker 单独部署后才在线上可用。
...
将旧的 AI 雷达 workflow 统一为 `ai-briefing`。保留旧触发词兼容，包括：
...
- 更新 `.opencode/commands/ai-radar.md` 指向新的 `ai-briefing` 语义。
...
7. 写入建议路径：`content/ai-briefings/YYYY-MM-DD-ai-briefing.md`。
