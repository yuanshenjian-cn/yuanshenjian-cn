...
- **日期**: 2026-05-09
- **状态**: 待实现
- **项目**: 个人博客 / `skills/investment-daily-briefing` / `.opencode/skills/investment-daily-briefing` / `app/investment`

## 1. 背景

当前仓库已经存在一套相对成熟的 `ai-daily-briefing` 体系，它不仅是一个 skill，还包括独立内容域、页面路由、公开索引、构建产物与发布门禁。现在需要在此基础上新增一个独立的 `投资` 内容域，并配套一套可手动触发、可严格审核、可自动发布到博客的 `investment-daily-briefing` skill。
...
2. 新增 `investment-daily-briefing` skill，保留 `查询 / 成稿 / 发布` 三段式 workflow。
3. 支持将投资每日简报正式发布到博客的新内容域。
...
- `/investment/daily-briefings/[date]`
- `/investment/[column]`
...
- 投资每日简报：`content/investment-briefings/`
...
```text
skills/investment-daily-briefing/
├── SKILL.md
├── README.md
├── references/
│   ├── source-map.md
...
当前工程同时保留 `skills/` 与 `.opencode/skills/` 两套 skill 目录约定。为与现有 `ai-daily-briefing` 保持一致，第一版应同时创建：

- `skills/investment-daily-briefing/`
- `.opencode/skills/investment-daily-briefing/`
...
1. `investment-daily-briefing` 发布模式通过审核
2. 写入 `content/investment-briefings/YYYY-MM-DD-investment-daily-briefing.md`
...
- `skills/investment-daily-briefing/` 与 `.opencode/skills/investment-daily-briefing/`：新增 skill 资产。

### 17.2 静态导出约束

由于项目使用静态导出，以下页面必须符合静态构建约束：

- `/investment/daily-briefings/[date]` 必须提供 `generateStaticParams()`，并设置 `dynamicParams = false`。
- `/investment/daily-briefings/archive/[year]/[month]` 必须提供 `generateStaticParams()`，并设置 `dynamicParams = false`。
...
4. 投资每日简报拥有独立列表、归档、详情路由，并能静态构建。
5. 正式简报写入 `content/investment-briefings/`，投资专栏长文继续走 `content/blog/investment/...`。
6. `investment-daily-briefing` 保留 `查询 / 成稿 / 发布` 三段式 workflow。
...
12. 正常版字数为 `1200 ~ 1500`，短版字数为 `900 ~ 1199`，不足 `900` 阻断发布。
