...
1. 校验脚本直接读取当前最新的 `bodyMin/bodyMax` 作为全量历史稿件的统一门槛。
2. 新规则发布后，历史简报会被拿来按新门槛重新审判，导致 CI 对存量内容产生回溯性破坏。
3. 去重规则已经有 `dedupeEffectiveFrom`，但字数规则此前没有对应的生效日期概念。

### 修复

1. 为 AI / 投资简报字数规则新增 `bodyRuleEffectiveFrom`。
2. AI 简报在 `2026-05-26` 之前继续按历史区间 `700~1100` 校验；从该日期起再按 `900~1300` 校验。
3. 投资简报在 `2026-05-26` 之前继续按历史区间 `900~1500` 校验；从该日期起再按 `1100~1700` 校验。
4. 该生效日期写入 `skills/*/config/briefing.json`，并同步镜像 skill 配置，避免后续再次漂移。
