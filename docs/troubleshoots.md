...
## 2026-05-05 页面级 AI 回答措辞过度页面化，影响作者页与文章页回答自然度

### 现象

- 作者页 AI 在岗位匹配、能力归纳类回答中，经常被 prompt 强制写成 `根据当前作者页展示的信息...`
- 文章页 AI 也容易出现 `页面中显示...`、`当前文章页展示的信息...` 这类过强的页面化措辞
- 虽然事实依据和引用 sectionIds 正常，但回答读起来不像在基于结构化内容或文章正文作答

### 根因

1. `blog-ai-worker/src/prompts/author.ts` 里把岗位匹配类回答约束成了必须以 `根据当前作者页展示的信息` 开头
2. `blog-ai-worker/src/prompts/article.ts` 没有显式约束文章页回答优先使用贴近正文的表达

### 修复

1. 移除作者页 prompt 中“必须以 `根据当前作者页展示的信息` 开头”的硬性要求
2. 作者页改为优先使用更贴近结构化来源的措辞，例如：
   - `作者技能中提到...`
   - `作者项目经验中提到...`
   - `作者经历中提到...`
   - `作者证书信息显示...`
3. 文章页补充正文导向措辞约束，优先使用：
   - `文章中提到...`
   - `文中提到...`
   - `本文介绍了...`
4. 明确保留原有事实依据边界和 `sectionIds` 引用逻辑，只调整回答风格约束

### 如何确认修复生效

1. 运行作者页与文章页相关测试：
   - `npm run test -- tests/blog-ai-worker/author-scene.test.ts tests/blog-ai-worker/article-scene.test.ts`
2. 确认作者页 prompt 里不再要求以 `根据当前作者页展示的信息` 开头
3. 确认作者页 prompt 包含结构化来源措辞示例
4. 确认文章页 prompt 包含 `文章中提到... / 文中提到... / 本文介绍了...` 这类正文导向措辞
5. 冒烟提问作者页和文章页，确认回答不再优先出现 `页面中显示...`、`当前作者页展示的信息...`、`当前文章页展示的信息...`
