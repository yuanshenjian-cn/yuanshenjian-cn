...
3. 回归运行：
   ...
   - `npm --prefix blog-ai-worker run typecheck`
## 2026-05-08 AI 每日简报作为独立内容域，避免污染普通博客流

## 问题

AI 每日简报每天生成一篇，若直接放入 `content/blog`，会自动进入首页最新文章、`/articles`、RSS、全站搜索和首页 AI 推荐，长期会稀释普通博客文章流。

## 处理

1. 使用独立目录 `content/ai-briefings` 存放 `.md` 简报。
2. 使用 `lib/briefings.ts` 独立读取简报，不复用普通文章流。
3. `scripts/build-ai-data.js` 生成独立的 `public/ai-data/briefings/index.json`，供 `/ai/daily-briefings` 内部 AI 推荐使用。
...
## 2026-05-08 AI 每日简报列表避免随着历史累积无限变重

### 问题

`/ai/daily-briefings` 如果长期一次性加载全部历史简报，不仅列表 HTML 会持续变大，还会把越来越多的简报数据传给推荐组件，导致页面负载和客户端序列化成本一起增长。

### 处理

1. 主列表页只读取最近 30 天简报。
2. 新增 `/ai/daily-briefings/archive` 与 `/ai/daily-briefings/archive/[year]/[month]`，让更早内容按自然月份分开浏览。
...
`/investment/daily-briefings` 需要接入页面顶部 AI 推荐，但如果直接复用现有 `briefing-recommend` scene，会同时踩到数据源、提示词边界和前端文案三处错位。

### 处理

1. 前端新增独立的 `InvestmentBriefingRecommendWidget`，文案明确只推荐投资每日简报，并标注“不构成投资建议”。
...
## 2026-05-09 投资每日简报时间窗口要按执行时点回溯 24 小时，且正文汉字数不能把来源章节算进去
...
## 2026-05-09 AI 每日简报的字数统计也必须排除 `## 来源` 章节，而不是只排除来源 URL

### 现象

- AI skill 文档里原先写的是“不含 frontmatter 和来源 URL”
- 这会让人误解为：只排除链接字符串本身，`## 来源` 章节里的其它文字仍然算字数
- 同时，AI 每日简报的校验逻辑此前也是直接统计整段正文，没有排除 `## 来源` 章节
...
1. `scripts/validate-post.js` 中 AI 每日简报字数统计改为先移除 `## 来源` 章节，再统计汉字数
...
1. 运行：
   - `npm run validate-content`
2. 确认 AI 每日简报命中字数错误时，报错文案为：
   - `AI 每日简报正文汉字数（不含来源章节）应为 700~1100`
...
## 2026-05-09 本地自动运行 AI 每日简报脚本时，不要依赖 macOS 默认没有的 `flock` 和 `stdbuf`

### 现象

- 本地写了一个 `scripts/ai-daily-briefing.sh`，希望用它自动调用 Claude CLI 生成并发布 AI 每日简报
...
- 默认安全模式运行：
  - `SKIP_PERMISSIONS=0 ./scripts/ai-daily-briefing.sh`
- 只有在你明确接受自动写文件 / commit / push 风险时，才开启：
  - `SKIP_PERMISSIONS=1 ./scripts/ai-daily-briefing.sh`

### 补充

- 如果是定时任务联调，建议优先使用：
  - `DRY_RUN=1 ./scripts/ai-daily-briefing.sh`
- 该模式会把 prompt 切换为“只起草、不发布”，默认同时把 Claude 的 `permission-mode` 切到 `plan`
- 如果你希望 dry-run 仍然真实执行更多步骤，而不只是规划，可以手工覆盖：
  - `DRY_RUN=1 PERMISSION_MODE=default ./scripts/ai-daily-briefing.sh`
