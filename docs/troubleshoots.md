...
## 2026-06-14 Turnstile 预加载失败后会卡住 AI 对话验证
...
```text
Turnstile 脚本加载失败，请稍后刷新重试。
```

用户即使稍后再次点击发送，也可能继续命中同一个失败状态。

### 根因

多个前端入口各自手写 Turnstile 脚本加载逻辑。预加载阶段如果脚本网络加载失败，页面里可能残留一个坏的 `script` 节点，后续正式请求继续复用这个节点或等待同一段失败状态，导致无法自动恢复。

### 修复

抽出 `site/lib/turnstile.ts` 作为共享 loader：

1. 统一 AI 对话、AI 推荐、简报推荐、投资简报推荐和评论区的脚本加载入口
2. 给 Turnstile `script` 标记 `loading` / `loaded` / `error` 状态
3. 发现 `error` 或无可用 `window.turnstile` 的旧脚本时，移除后重新插入脚本
4. 补充回归测试，覆盖“首次预加载失败，正式请求成功重试”的路径

### 结论

第三方验证脚本不能只做一次性注入。预加载失败应静默吞掉，但正式用户动作必须具备清理坏脚本并重试的能力，否则临时网络失败会被放大成整页会话不可用。
...
## 2026-06-03 未配置 `CORE_SERVICE_DATABASE_URL` 时，`sync-content` 不应把整次 push 标红

### 现象

GitHub Actions 的 `Sync · Content` workflow 在 `Sync public content` 步骤失败，报错类似：
...
`Sync · Content` 在当前项目里应视为“按需启用”的生产能力，而不是主站/后端基础部署的阻断项。没配 RAG 所需 secrets 时，workflow 应跳过而不是失败。

## 2026-06-03 `sync-content` 的 `CORE_SERVICE_DATABASE_URL` 必须使用 asyncpg 风格 URL
...
`sync-content` 会导入 `core-service/app/shared/infra/database.py`，当前项目已统一使用 async SQLAlchemy：
...
`sync-content.yml` 已新增前置校验步骤，会明确阻断以下问题：
