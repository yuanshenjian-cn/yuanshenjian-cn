# AI 简报 Implementation Plan

> Steps use checkbox (`- [ ]`) syntax for progress tracking.

**Goal:** 在现有 AI 入口中加入独立的 AI 简报内容域、往期列表、详情页、简报域内 AI 推荐，并把旧的 AI 雷达 workflow 统一为默认自动发布的 `ai-briefing` skill。

**Architecture:** AI 简报使用独立 `content/ai-briefings` 内容目录和 `lib/briefings.ts` 数据层，避免进入普通博客流。页面路由为 `/ai` 顶部最新卡、`/ai/briefings` 列表和 `/ai/briefings/[date]` 详情；Worker 新增 `briefing-recommend` scene，只读取 `public/ai-data/briefings/index.json`。
...
- Create: `content/ai-briefings/2026-05-08-ai-briefing.md` 示例简报，用于首版页面和构建验证。
- Create: `lib/briefings.ts`，负责读取、解析、排序 AI 简报。
- Create: `types/briefing.ts`，定义简报类型。
- Create: `components/briefings/briefing-recommend-widget.tsx`，列表页顶部专属推荐入口。
- Create: `app/ai/briefings/page.tsx`，往期列表页。
- Create: `app/ai/briefings/[date]/page.tsx`，简洁详情页。
- Modify: `app/ai/page.tsx`，顶部增加最新一期简报摘要卡，下方保留 AI 专栏卡。
- Modify: `app/sitemap.ts`，加入 `/ai/briefings` 和详情页 URL。
...
- Modify: `.opencode/commands/ai-radar.md`，兼容旧命令但指向 AI 简报语义。
- Create: `.opencode/skills/ai-briefing/SKILL.md`，新 skill。
- Copy/update: `.opencode/skills/ai-briefing/references/source-map.md` 和 `evals/evals.json`。
- Remove/replace: 旧 AI 雷达 skill 入口，统一切换到 `ai-briefing`。
...
- Create: `content/ai-briefings/2026-05-08-ai-briefing.md`
...
- Create: `app/ai/briefings/page.tsx`
- Create: `app/ai/briefings/[date]/page.tsx`
- Modify: `app/sitemap.ts`

- [ ] **Step 1: Add latest briefing card to `/ai`**

Place a horizontal “最新一期 AI 简报” card above the existing AI 专栏 grid, including the empty state from the design.

- [ ] **Step 2: Create `/ai/briefings` list page**
...
Add `/ai/briefings` and all published briefing details. Do not add RSS entries.
...
**Files:**
- Create: `.opencode/skills/ai-briefing/SKILL.md`
- Create/copy: `.opencode/skills/ai-briefing/references/source-map.md`
- Create: `.opencode/skills/ai-briefing/evals/evals.json`
- Modify: `.opencode/commands/ai-radar.md`

- [ ] **Step 1: Create `ai-briefing` skill**
...
Record why AI 简报 uses `content/ai-briefings` and does not enter ordinary article flows.
