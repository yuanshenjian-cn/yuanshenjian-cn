...
| 输出文件 | `skills/blog-glossary-sync/output/glossary-candidates.json`（候选列表，供 review） |
| 脚本文件 | `skills/blog-glossary-sync/output/sync-glossary.sh`（本地）、`skills/blog-glossary-sync/output/sync-glossary-prod.sh`（生产） |
| 认证方式 | 读取环境变量 `ADMIN_API_KEY` 和 `CORE_SERVICE_URL` |
...
- 检查 `ADMIN_API_KEY` 和 `CORE_SERVICE_URL` 是否已设置；未设置则提示用户，不继续。
- 确认后端可访问：`curl -s "$CORE_SERVICE_URL/api/v1/healthz"` 或任意公开接口。
...
写入 `skills/blog-glossary-sync/output/glossary-candidates.json`，结构示例：
...
根据当前环境生成对应脚本。本地环境写入 `skills/blog-glossary-sync/output/sync-glossary.sh`，生产环境写入 `skills/blog-glossary-sync/output/sync-glossary-prod.sh`。两个脚本都通过环境变量读取 `ADMIN_API_KEY`，不会在文件中硬编码密钥。
...
export CORE_SERVICE_URL="http://localhost:8001"
export ADMIN_API_KEY="$(grep '^ADMIN_API_KEY=' core-service/.env | cut -d= -f2)"
python3 skills/blog-glossary-sync/scripts/render_sync_script.py \
...
# 创建新术语
curl -s -X POST "$CORE_URL/api/v1/admin/knowledge-terms" \
...
# 更新已有术语
curl -s -X PUT "$CORE_URL/api/v1/admin/knowledge-terms/{id}" \
  -H "Content-Type: application/json" \
  -H "Origin: $ORIGIN" \
  -H "Authorization: Bearer $ADMIN_API_KEY" \
  -d '{...}'
...
明确告诉用户：
- `skills/blog-glossary-sync/output/glossary-candidates.json` 可手动编辑。
- 本地执行：`bash skills/blog-glossary-sync/output/sync-glossary.sh`
- 生产执行：`bash skills/blog-glossary-sync/output/sync-glossary-prod.sh`
- 若需要重新生成，可再次调用本 skill。

**在收到用户明确说“执行”“运行脚本”“写入”之前，不要执行同步脚本。**
...
执行后检查输出，如有 401/403/429 等错误，向用户解释原因（常见：API key 错误、Origin 不匹配、Cloudflare WAF 拦截、限流）。
...
- 生成脚本后建议 `chmod +x skills/blog-glossary-sync/output/sync-glossary*.sh`（`render_sync_script.py` 已自动处理）。
- 使用 `ADMIN_API_KEY` 调用术语接口不受 admin 登录限流影响。
- 同步到生产环境前，建议先在本地 `core-service` 验证一次。
