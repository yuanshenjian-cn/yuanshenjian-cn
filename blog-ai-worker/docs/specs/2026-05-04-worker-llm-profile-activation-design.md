...
- `provider`：运行时 provider 工厂选择键，例如 `deepseek`
- `modelKey`：本地 profile 选择键，例如 `deepseek-v4-pro`
...
- `LLM_ACTIVE_PROFILE`：当前 profile key，例如 `deepseek/deepseek-v4-pro`
- `LLM_PROVIDER_NAME`：当前 provider 名，例如 `deepseek`
- `LLM_MODEL_ID`：当前真实模型名，例如 `deepseek-v4-pro`
...
未来如需接入其他 OpenAI-compatible provider，则在 provider 工厂中新增对应分支即可。
...
```text
providers.deepseek.models.deepseek-v4-pro.modelId is required
```
...
3. `npm run llm:use -- deepseek/deepseek-v4-pro` 能把该 profile 记录为当前本地 active profile。
4. `npm run llm:deploy -- deepseek/deepseek-v4-pro` 能完成线上激活并部署。
