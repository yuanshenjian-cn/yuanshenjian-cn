...
import {
  buildInvestmentBriefingRecommendStreamSystemPrompt,
  INVESTMENT_BRIEFING_RECOMMEND_REFERENCE_DELIMITER,
} from "../prompts/investment-briefing-recommend";
...
const INVESTMENT_BRIEFING_RECOMMEND_AI_ERROR_MESSAGE = "AI 服务刚刚开小差了，请稍后重试。";
const MAX_CONTEXT_BRIEFINGS = 10;
const MAX_REFERENCES = 3;
const QUERY_STOPWORDS = ["请", "推荐", "简报", "动态", "内容", "关于", "一下", "最近", "投资", "观察"];

interface InvestmentBriefingIndexItem {
  slug: string;
  title: string;
  brief: string;
  tags: string[];
...
function isIndexItem(value: unknown): value is InvestmentBriefingIndexItem {
  if (!isRecord(value)) {
    return false;
  }
...
function isInRange(date: string, range: "3d" | "7d" | "14d" | "30d"): boolean {
...
function parseTailPayload(value: string): InvestmentBriefingRecommendTailPayload {
  const payload: unknown = JSON.parse(value);

  if (!isRecord(payload) || !Array.isArray(payload.slugs) || !payload.slugs.every((slug) => typeof slug === "string")) {
    throw new Error("Invalid investment briefing recommend tail payload");
  }
...
function toRecommendReference(item: InvestmentBriefingIndexItem): RecommendReference {
  return {
    slug: item.slug,
    title: item.title,
    excerpt: item.brief,
...
async function fetchIndex(env: Env): Promise<RecommendReference[]> {
  const response = await fetch(buildInvestmentBriefingIndexUrl(env));
...
export async function handleInvestmentBriefingRecommendSceneStream(
  ...
  const rangedBriefings = index.filter((briefing) => isInRange(briefing.date, range));
  ...
  try {
    upstreamStream = await provider.streamChat(createChatRequest(buildInvestmentBriefingRecommendStreamSystemPrompt(briefings), message));
  } catch (error) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const delimiterIndex = fullText.indexOf(INVESTMENT_BRIEFING_RECOMMEND_REFERENCE_DELIMITER);
        const answer = delimiterIndex >= 0 ? fullText.slice(0, delimiterIndex) : fullText;
        const tail = delimiterIndex >= 0 ? fullText.slice(delimiterIndex + INVESTMENT_BRIEFING_RECOMMEND_REFERENCE_DELIMITER.length) : "";
        ...
        controller.enqueue(encodeSSEEvent("error", { message: INVESTMENT_BRIEFING_RECOMMEND_AI_ERROR_MESSAGE }));
        controller.close();
      }
    },
  });
