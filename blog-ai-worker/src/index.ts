...
import { handleBriefingRecommendSceneStream } from "./scenes/briefing-recommend";
import { handleInvestmentBriefingRecommendSceneStream } from "./scenes/investment-briefing-recommend";
...
async function parseBody(request: Request, env: Env): Promise<ChatRequestBody> {
  ...
  if (
    ...
    (
      ...
      payload.scene !== "briefing-recommend" &&
      payload.scene !== "investment-briefing-recommend" &&
      payload.scene !== "article" &&
      payload.scene !== "author"
    ) ||
  ...
  if (payload.scene === "briefing-recommend") {
    if (!isRecord(payload.context)) {
      throw new HttpError(400, "Invalid request payload");
    }

    if (
      payload.context.range !== "today" &&
      payload.context.range !== "3d" &&
      payload.context.range !== "7d" &&
      payload.context.range !== "14d" &&
      payload.context.range !== "30d"
    ) {
      throw new HttpError(400, "Invalid request payload");
    }
  ...
  if (payload.scene === "investment-briefing-recommend") {
    if (!isRecord(payload.context)) {
      throw new HttpError(400, "Invalid request payload");
    }
  ...
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (routeType === "stream") {
        ...
        if (body.scene === "briefing-recommend") {
          return await handleBriefingRecommendSceneStream(body.message, body.context.range, env, origin);
        }

        if (body.scene === "investment-briefing-recommend") {
          return await handleInvestmentBriefingRecommendSceneStream(body.message, body.context.range, env, origin);
        }
        ...
        throw new HttpError(400, "Streaming is only supported for recommend, briefing-recommend, investment-briefing-recommend, article and author scenes");
      }
