import type {
  AIChatStreamOptions,
  AIUsage,
  BriefingRecommendChatStreamOptions,
  InvestmentBriefingRecommendChatStreamOptions,
  PageReference,
  PageResponse,
  PageStreamEvent,
...
export async function aiBriefingRecommendStream(options: BriefingRecommendChatStreamOptions): Promise<void> {
  const workerUrl = options.workerUrl.replace(/\/$/, "");
  const response = await fetch(`${workerUrl}/chat/stream`, {
    method: "POST",
    headers: {
...
export async function aiInvestmentBriefingRecommendStream(options: InvestmentBriefingRecommendChatStreamOptions): Promise<void> {
  const workerUrl = options.workerUrl.replace(/\/$/, "");
  const response = await fetch(`${workerUrl}/chat/stream`, {
    method: "POST",
    headers: {
