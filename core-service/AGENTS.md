...
- 应用级配置（CORS、限流参数、AI provider/profile、限流参数）放在 `app/config.yml`，按 `default` / `local` / `test` / `production` 分段；常用字段可以通过环境变量覆盖。
- LLM provider / profile 配置统一放在 `app/config.yml` 的 `ai.providers` 中，不直接在 provider 代码中写 model ID 字符串。
