...
2. `app/application.yml`：结构化运行配置（CORS、cookie、AI provider/profile、限流参数）
...
- `core-service/app/application.yml`：分环境结构化配置，包含 LLM provider / model / scene 映射
...
- 默认 cookie domain：本地环境为空，生产从 `app/application.yml` 读取 `.yuanshenjian.cn`
- 默认管理员口令：`admin123456`
...
LLM provider、model 和 scene 映射统一维护在 `core-service/app/application.yml`。真实 API key 不写进 YAML，使用 `${DEEPSEEK_API_KEY}` 这类占位符从 `.env.local` 或 Render Env 注入。

### `application.yml` 的环境变量覆盖

`app/application.yml` 提供默认值；生产或本地可以用环境变量覆盖常用字段。LLM provider 列表统一由 `ai.providers` 定义，不再单独维护 JSONC 配置文件。
...
1. `app/application.yml`：provider / model / scene profile 定义
2. `.env.*` / Render env：真实 API key

例如 `app/application.yml` 中的 provider 配置：
