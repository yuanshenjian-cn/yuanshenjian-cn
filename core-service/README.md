...
```bash
just start-local-pgvector-postgres
...
- 本地数据库：从 `core-service/.env.local` 或当前 shell 的 `DATABASE_URL` 读取
- 默认 cookie domain：本地环境为空，生产从 `app/config.yml` 读取 `.yuanshenjian.cn`
- 默认管理员口令：`admin123456`

说明：通过 `just` 启动本地后端时会注入默认 `SESSION_SECRET` 和 `ADMIN_SECRET_HASH`，但不会覆盖 `DATABASE_URL`。数据库连接串请放在 `core-service/.env.local` 或当前 shell 环境里。推荐先执行 `just start-local-pgvector-postgres`，再把输出的连接串写入 `core-service/.env.local`。默认管理员口令为 `admin123456`。
...
```bash
DATABASE_URL=postgresql+asyncpg://postgres:postgres@127.0.0.1:5437/ysj_blog
