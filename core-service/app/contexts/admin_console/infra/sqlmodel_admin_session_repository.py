...
class SQLModelAdminSessionRepository(AdminSessionRepository):
    ...
    async def add(self, admin_session: AdminSession) -> AdminSession:
        ...
        await self._admin_session_dao.flush()
        await self._admin_session_dao.refresh(admin_session_po)
        return self._to_domain(admin_session_po)

    async def get_by_session_hash(self, session_hash: str) -> AdminSession | None:
        admin_session_po = await self._admin_session_dao.get_by_session_hash(session_hash)
    ...
    async def save(self, admin_session: AdminSession) -> AdminSession:
        admin_session_po = await self._admin_session_dao.get_by_session_hash(admin_session.session_hash)
