...
def test_knowledge_term_query_service_supports_exact_scene_domain_filters() -> None:
    async def run() -> None:
        engine = create_async_engine("sqlite+aiosqlite:///:memory:")
...
def test_knowledge_term_query_service_skips_count_when_not_requested() -> None:
    async def run() -> None:
        engine = create_async_engine("sqlite+aiosqlite:///:memory:")
        knowledge_term_table = cast(Table, KnowledgeTermPO.__table__)
        async with engine.begin() as connection:
